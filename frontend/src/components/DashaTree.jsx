...
function Row({node,depth,children,defaultOpen}){
...
export default function DashaTree({dasha,lang='en'}){
  ...
  return <div>
    ...
    {dasha.sequence.map(maha=>(
      <Row key={maha.lord+maha.start} node={maha} depth={0} defaultOpen={isNow(maha.start,maha.end)}>
        {maha.antar.map(an=>(
          <Row key={an.lord+an.start} node={an} depth={1} defaultOpen={isNow(an.start,an.end)}>
            {an.pratyantar.map(pr=>(<Row key={pr.lord+pr.start} node={pr} depth={2}/>))}
          </Row>))}
      </Row>))}
  </div>
}
